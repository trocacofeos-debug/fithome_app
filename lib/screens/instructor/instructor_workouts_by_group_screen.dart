...
enum _FiltroTreino { todos, publicados, rascunhos }

/// Mostra só os treinos de um grupo específico: um aluno (treinos
/// individuais dele) ou "Geral" (treinos sem dono, visíveis a todos).
class InstructorWorkoutsByGroupScreen extends StatefulWidget {
  final String? alunoId; // null = grupo "Geral"
  final String tituloGrupo;

  const InstructorWorkoutsByGroupScreen({
    super.key,
  ...
  @override
  State<InstructorWorkoutsByGroupScreen> createState() => _InstructorWorkoutsByGroupScreenState();
}

class _InstructorWorkoutsByGroupScreenState extends State<InstructorWorkoutsByGroupScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(widget.tituloGrupo, style: condensed(fontSize: 18))),
      floatingActionButton: FloatingActionButton.extended(
        backgroundColor: AppColors.instrutorColor,
        foregroundColor: Colors.white,
        onPressed: () {
          final query = widget.alunoId != null
              ? '?alunoId=${widget.alunoId}&alunoNome=${Uri.encodeComponent(widget.tituloGrupo)}'
              : '';
          context.push('/instrutor/novo-treino$query');
        },
        ...
        children: [
          ...
          StreamBuilder(
            ...
            builder: (context, snapshot) {
              ...
              final doGrupo = snapshot.data!.where((t) => t.alunoId == widget.alunoId).toList();
              final treinos = _aplicarFiltros(doGrupo);

              if (treinos.isEmpty) {
                return Padding(
                  padding: const EdgeInsets.only(top: 30),
                  child: Text(
                    doGrupo.isEmpty ? 'Nenhum treino aqui ainda.' : 'Nenhum treino encontrado.',
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                        '${treino.duracaoMinutos} min · ${treino.nivel}'
                        '${treino.diaDaSemana != null ? " · ${rotuloDiaDaSemana(treino.diaDaSemana!)}" : ""}',
